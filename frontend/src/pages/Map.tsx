...
import { Map as MapIcon, Navigation2, Compass, Loader2 } from 'lucide-react';
...
export const Map = () => {
  ...
  const handleCalculateRoute = async () => {
    ...
    try {
      const res = await MapService.calculateRoute('Current Location', destination);
      setToast({ message: res.message || 'Route calculated successfully', type: 'success' });
    } catch (err) {
      setToast({ message: 'Failed to calculate route', type: 'error' });
    } finally {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 h-full relative">
              ...
              <div className="relative">
                ...
                <div className="flex gap-3 items-center relative z-10">
                  ...
                  <input 
                    ...
                    className="w-full bg-automotive-black border border-automotive-gray/30 rounded p-2 text-xs text-automotive-white outline-none placeholder:text-automotive-gray/50" 
                  />
                </div>
              </div>
              <button 
                ...
                className="w-full bg-automotive-blue/20 hover:bg-automotive-blue/30 text-automotive-blue border border-automotive-blue/50 py-2.5 rounded uppercase tracking-widest text-xs font-bold transition-colors mt-2 flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isCalculating ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Calculate Route'}
              </button>
            </div>
          </BaseCard>
